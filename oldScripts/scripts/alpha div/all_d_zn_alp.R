...
qa<- select(q, -c(id, e, "Genotype", Treatment, Round, Diet, DT, type))
...
colnames(x1)[1] <- "Shannon"

#anova
w1<- x1[(x1$Treatment=="NS"),]
...
write.csv(y5, "alld_all_zn_alpd_d.csv")

u1<-cbind(t1,t2,t3)
u2<-u1[, -c(3,5) ]
u2$DT <- gsub('13', '11', u2$DT)

write.csv(u2, "alld_all_zn_alpd_perd.csv")