...
ggsave("alld_R_s_braymds_allz.png", dpi = 300, width=25, height=25, units= c("cm"))
...
ggsave("alld_P_s_braymds_allz.png", dpi = 300, width=25, height=25, units= c("cm"))
...
a<- q1[(q1$"DT"=="PHFAS" |q1$"DT"=="PHSA6S" | q1$"DT"=="PAS" | q1$"DT"=="PHSA11S" ),]
b<- a[, -c(1:8) ]
c<-vegdist(b, method="bray", binary=FALSE, diag=FALSE, upper=FALSE)
z<-as.matrix(c)
z1<-as.data.frame(z)
...
ggsave("alld_PA_s_braymds_allz.png", dpi = 300, width=25, height=25, units= c("cm"))