...
n_d_wunif_z <- read_excel("C:/Users/abomb/Box/HF and HS/R/HF HS/input/n_d_wunif_z.xlsx")
...
write.csv(i4, "n_wunif_anos_allz.csv")

### Diet
b<- q1[(q1$"DT"=="RNS" | q1$"DT"=="PRNS"),]
a<-as.vector(b$`Column 1`)
w<- as.data.frame(b[, a])
e1 <- as.vector(b$"DT")
r<- anosim(w, e1, permutations = 999)
...
i7<-rbind(i1,i2,i3,i4,i5,i6)

write.csv(i7, "n_wunif_anos_d_allz.csv")