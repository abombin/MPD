...
a$DT <- factor(a$DT, level=c("RHFNS","RHFS","RHSNS", "RHSS", "PHFNS", "PHFS","PHSNS", "PHSS", "PHFANS", "PHFAS", 
                             "PHSA6NS", "PHSA6S", "PHSA11NS" ))

# plot
ggplot(cbind(a, iris.lda.values$x),
       aes(y = LD1, x = LD2, colour = DT)) + 
  geom_point(aes(color=DT), size=5) +
...
ggsave("hfhs_disc_100z_norm_alldt_ind.png", dpi = 300, width=25, height=25, units= c("cm"))